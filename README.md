# Contract to Core

An educational onboarding simulation for a junior contract AI Business Analyst
joining an insurance automation team.

The player completes 18 workplace scenarios, chooses A/B/C/D, receives tailored
manager feedback, and builds six capability signals that influence a
deterministic permanent-role outcome.

## Run locally

```bash
npm install
npm run dev
```

## Validate

```bash
npm test
npm run lint
npm run test:e2e
```

## Product notes

- Progress is stored only in the browser with `localStorage`.
- Hints are educational and do not reduce the score.
- The simulation is based on the accompanying onboarding plan and vault-derived
  learning principles.
- It is not official AIA guidance or an employment forecast.
